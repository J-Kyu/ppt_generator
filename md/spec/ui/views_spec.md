...
- **주요 Flet 컴포넌트:** 
  ...
  - `ft.Container`: 커다란 사각형 영역의 Drag & Drop Zone 생성 (UI/UX 디자인 요소 강화)
  - `ft.ProgressRing` / `ft.ProgressBar`: 파일이 업로드되고 파싱될 때 돌아가는 로딩 애니메이션
- **동작 흐름(Action):**
  1. 파일(템플릿)이 드래그 앤 드롭되거나 선택되면 화면에 스피너를 돌립니다.
  2. 파일의 절대 경로를 `core.engine.analyze_ppt()`에 넘겨 분석을 요쳥합니다.
  3. 정상적으로 반환된 전체 레이아웃 스키마 목록을 전역 상태인 `app_state`에 저장합니다.
  4. 로딩 스피너 작동을 종료하고 `Layout Picket View` (단계 2)로 화면 라우팅을 이동합니다.
